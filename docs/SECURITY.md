...
- local document extraction is a bounded offline path only:
  - `docling==2.122.0` is the only canonical document engine v1; `onnxruntime` is present strictly as the ONNX inference runtime for the local RapidOCR path, not as a second document engine;
  ...
  - local Docling model assets (layout model plus RapidOCR PP-OCRv5 cyrillic rec/det/dict) are prepared explicitly via `config/start.py docling-assets-prepare`; runtime runs offline (`HF_HUB_OFFLINE=1`) and never silently downloads models; missing model/OCR prerequisites degrade explicitly and never fall back to AI file reading;
