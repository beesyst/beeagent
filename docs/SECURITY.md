...
- Write-back is disabled by default; enabling it requires `bitrix.enabled: true`, `bitrix.reconciliation.enabled: true`, non-empty configured customer Lead `stageId` values for `new_lead`, `new_lead_assigned` and `irrelevant`, and the write credential env var. Invalid or missing stage config fails fast and means zero mutations.
...
- Unresolved/ambiguous/inactive/degraded or malformed responsible evidence, unsafe/ambiguous/duplicate reconciliation targets, conflicting thread references and unresolved `existing_deal`/`duplicate` fail closed to an explicit `deferred` outcome; no speculative Lead is created and existing CRM entities are never reassigned. An exact active routing match with a positive `user_id` sets a new Lead `ASSIGNED_BY_ID` and, for a `new_lead`, uses the configured `bitrix.writeback.stages.new_lead_assigned` stage; when configured, `bitrix.writeback.user_id_fallback` (a positive Bitrix user ID, validated fail-fast) is used only for `not_found` and keeps the configured `new_lead` stage. Matched, ambiguous, `connector_degraded`, unresolved and not-attempted routing stay deferred and the fallback never overrides an exact matched responsible.
