...
Write-back is disabled by default (`bitrix.writeback.enabled: false`) and performs zero writes until it is explicitly enabled with a dedicated env credential and configured customer Lead `stageId` values for `new_lead`, `new_lead_assigned` and `irrelevant` (all required fail-fast when enabled).
