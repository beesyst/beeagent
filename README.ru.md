...
- fail closed: unresolved/inactive/ambiguous/degraded responsible, ambiguous/unsafe/duplicate target, unresolved `existing_deal`/`duplicate` → `deferred` без спекулятивного create; exact active routing match с positive `user_id` authorizes Lead creation; optional `bitrix.writeback.user_id_fallback` назначает указанного пользователя только при `responsible.status=not_found`; existing CRM entity никогда не reassign;
...
- config-driven storage bounds: `rop.attachments.storage.{enabled,file_max_bytes,message_aggregate_max_bytes,files_max_per_message}` (fail-fast validated); required storage failure блокирует checkpoint; oversized/count/aggregate/blocked (`.eml`/`message/rfc822`) — explicit status; retention/download/Bitrix delivery независимы от AI analysis (`rop.attachments.enabled:false` = zero provider calls);
- bounded AI document understanding через configured file-capable provider path: `rop.attachments.analysis.{provider,file_capable,max_chars}`, только явно поддерживаемые типы/размеры, unsupported/provider failure — explicit degraded без local unsafe parser fallback, document instructions не дают execution authority, valid результат попадает в existing `attachment_text_preview`/extraction contract;
