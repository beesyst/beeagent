...
- bounded AI document understanding через configured file-capable provider path: `rop.attachments.analysis.{provider,file_capable,chars_max}`; OpenAI Responses uses `input_file` for PDF/DOCX and `input_image` data URLs for JPEG/PNG, and file capability remains disabled until controlled provider smokes prove the configured profile; unsupported/provider failure — explicit degraded без local unsafe parser fallback, document instructions не дают execution authority, valid результат попадает в existing `attachment_text_preview`/extraction contract;
